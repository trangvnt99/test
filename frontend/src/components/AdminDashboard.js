        ...
        <div className="flex min-h-screen bg-slate-50 font-sans antialiased text-slate-900">
            ...
            <aside className="fixed inset-y-0 left-0 w-64 bg-white border-r border-slate-200 z-50 hidden lg:flex flex-col">
                <div className="p-6 flex items-center gap-3 border-b border-slate-50">
                    ...
                    <h1 className="text-xl font-black tracking-tight text-slate-800">DITAGIS</h1>
                </div>

                <div className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-3 mb-4">Menu hệ thống</p>
            ...
            <main className="lg:ml-56 flex-1 flex flex-col min-w-0">
                ...
                <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 lg:px-10 flex items-center justify-between sticky top-0 z-40">
                    <div className="flex items-center gap-4">
                        <h2 className="text-lg font-bold text-slate-800 hidden md:block italic uppercase tracking-tighter">Hệ thống Quản trị</h2>
                    </div>

                    <div className="flex items-center gap-4">
                        <div className="text-right hidden sm:block">
                            <p className="text-sm font-bold text-slate-800 leading-none mb-1">{adminName}</p>
                ...
                <div className="p-6 lg:p-10 max-w-7xl mx-auto w-full">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6">
                        <div>
                            <h3 className="text-3xl font-black text-slate-900 mb-2 leading-tight">Tổng quan hệ thống</h3>
                            <p className="text-slate-500 font-medium">Bạn có <span className="text-blue-600">{articles.length} bài viết</span> đang hoạt động.</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                        <StatCard label="Tổng bài viết" value={articles.length} icon="bi-file-text" color="blue" />
                        <StatCard label="Lượt xem" value="2.4k" icon="bi-eye" color="emerald" />
                        <StatCard label="Dữ liệu" value="1.2GB" icon="bi-hdd-network" color="amber" />
                    </div>

                    {/* === NHÚNG COMPONENT DANH SÁCH BÀI VIẾT VÀO ĐÂY === */}
                    <div className="-mx-6 lg:-mx-10 mt-4">
                        {/* Mình dùng margin âm để component con bung rộng ra lấp đầy khoảng trống */}
                        <AdminArticleList />
                    </div>
...
function NavItem({ icon, label, active = false }) {
    return (
        <button type="button" className={`flex w-full items-center gap-4 px-4 py-3 border-0 rounded-xl font-bold transition-all duration-200 ${active
