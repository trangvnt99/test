...
const SolutionSection = ({ title, description, items, imageRight = false }) => (
  <div
    className="py-16 border-b border-slate-100 last:border-0"
    data-aos="fade-up"
  >
    <div
      className={`flex flex-col ${imageRight ? "lg:flex-row-reverse" : "lg:flex-row"} gap-12 items-center`}
    >
      ...
      <div className="lg:w-1/2 w-full">
        <div className="bg-slate-200 rounded-2xl h-[300px] flex items-center justify-center shadow-inner overflow-hidden">
          ...
          <div className="text-slate-400 italic text-sm text-center p-10">
            [Image: Minh họa mô phỏng mạng lưới cấp nước 3D / Phân tích viễn
            thám]
          </div>
        </div>
      </div>
...
const QLQHCapNuoc = () => {
  ...
  const t = languageData[lang].cap_nuoc;
  useEffect(() => {
    AOS.init({ duration: 1000, once: true });
  }, []);
  // Bảo vệ nếu t chưa load kịp
  if (!t) return null;
  return (
    <div className="min-h-screen bg-white font-sans antialiased text-slate-900">
      ...
      <section className="relative h-[450px] flex items-center justify-center">
        ...
        >
          ...
          <p className="text-blue-100 text-xl max-w-3xl mx-auto italic font-light">
            {/* "Sự kết hợp hoàn hảo giữa GIS & Viễn thám trong quản trị hạ tầng đô
            thị thông minh." */}
            {t.hero_sub}
          </p>
        </div>
        ...
        <div className="container mx-auto px-6 lg:px-20">
          ...
          >
            <p className="text-2xl text-slate-700 leading-relaxed font-medium">
              {/* Làm thế nào để kiểm soát hàng nghìn km đường ống ngầm dưới lòng
              đất mà không cần đào bới vô tội vạ? */}
              {t.intro_top}
            </p>
            <p className="text-lg text-slate-500">
              {/* Câu trả lời nằm ở sự kết hợp giữa <strong>GIS</strong> và{" "}
              <strong>RS</strong>. Hãy cùng khám phá bộ đôi đang thay đổi cuộc
              chơi quản lý đô thị. */}
              {t.intro_bot}
            </p>
          </div>
      ...
      <section className="container mx-auto px-6 lg:px-20 py-10">
        <SolutionSection
          ...
          description={t.section1.desc}
          items={t.section1.items}
        />
        ...
        <SolutionSection
          ...
          description={t.section2.desc}
          items={t.section2.items}
          imageRight={true}
        />
      </section>
      ...
      <section className="py-20 container mx-auto px-6 lg:px-20">
        <div
          className="bg-blue-600 rounded-3xl p-10 md:p-16 text-white flex flex-col md:flex-row justify-between items-center"
          data-aos="flip-up"
        >
          <div className="md:max-w-xl">
            ...
            <p className="text-blue-100 text-lg mb-8 leading-relaxed">
              {/* Chúng tôi cung cấp hệ thống "Digital Twin" hoàn hảo giúp các nhà
              quản lý đưa ra quyết định dựa trên dữ liệu thực (Data-driven). */}
              {t.product_desc}
            </p>
