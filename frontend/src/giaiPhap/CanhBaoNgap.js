...
const CanhBaoNgap = () => {
  ...
  const t = languageData[lang].canh_bao_ngap;
  useEffect(() => {
    ...
    <div className="min-h-screen bg-white font-sans antialiased text-slate-900">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <div data-aos="zoom-out">
            ...
            <p className="text-blue-200 text-xl md:text-2xl max-w-3xl mx-auto font-light leading-relaxed">
              {/* Giải pháp GIS thông minh giúp đô thị "kiên cường" hơn trước những
              cơn mưa cực đoan và biến đổi khí hậu. */}
              {t.hero_sub}
            </p>
          </div>
        </div>
      ...
      <section className="py-20">
        <div className="container mx-auto px-6 lg:px-20">
          <div
            className="max-w-4xl mx-auto border-l-4 border-blue-600 pl-8 py-2"
            data-aos="fade-right"
          >
            <p className="text-2xl text-slate-700 font-medium leading-relaxed italic">
              {/* "Trong những năm gần đây, 'ngập' đã trở thành nỗi ám ảnh kinh niên
              của các đô thị lớn. Khi phương pháp thủ công không còn đủ nhanh,
              GIS nổi lên như một vị cứu tinh giúp chúng ta chủ động ứng phó." */}
              {t.intro_quote}
            </p>
          </div>
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-16 items-center">
            <div className="lg:w-1/2" data-aos="fade-right">
              <h2 className="text-4xl font-bold mb-8 text-slate-800">
                {/* Lợi ích đột phá của GIS */}
                {t.benefits_title}
              </h2>
              <div className="space-y-8">
                {t.benefits_list.map((item, i) => (
                  <div key={i} className="flex gap-4">
                    <div className="w-6 h-6 rounded-full bg-blue-600 flex-shrink-0 mt-1 flex items-center justify-center text-white text-xs">
                      ✓
                    </div>
                    <div>
              ...
              <div className="relative p-4 bg-slate-900 rounded-3xl shadow-2xl overflow-hidden group">
                ...
                <div className="bg-slate-800 rounded-2xl h-[400px] border border-slate-700 flex flex-col p-6">
                  ...
                  <div className="flex-1 border border-slate-700 rounded-lg bg-slate-900/50 flex items-center justify-center text-slate-500 italic text-sm">
                    {lang === "vi"
                      ? "[Mô phỏng bản đồ số cảnh báo ngập]"
                      : "[Digital Flood Map Simulation]"}
                  </div>
      ...
      <section className="py-20 bg-blue-600 text-white overflow-hidden">
        <div className="container mx-auto px-6 lg:px-20">
          <div className="max-w-4xl mx-auto text-center" data-aos="zoom-in">
            <h2 className="text-3xl md:text-4xl font-bold mb-6 italic text-blue-100">
              {/* "Tương lai của quản lý ngập: GIS kết hợp AI" */}
              {t.vision_title}
            </h2>
            <p className="text-xl leading-relaxed text-white/90">
              {/* Sự kết hợp này mở ra một kỷ nguyên mới. AI sẽ học từ các trận ngập
              trong quá khứ để đưa ra kịch bản ứng phó chính xác đến từng con
              hẻm, giúp đô thị trở nên "kiên cường" hơn trước biến đổi khí hậu
              toàn cầu. */}
              {t.vision_desc}
            </p>
          </div>
      ...
      <section className="py-20 bg-slate-50 text-center">
        <div className="container mx-auto px-6" data-aos="fade-up">
          <p className="max-w-3xl mx-auto text-lg text-slate-500 italic leading-relaxed mb-10">
            {/* Hệ thống cảnh báo ngập bằng GIS là công cụ thiết yếu bảo vệ cộng
            đồng. Việc đầu tư và phát triển hệ thống này là chìa khóa để đối phó
            hiệu quả với thách thức thiên tai trong tương lai. */}
            {t.closing_desc}
          </p>
